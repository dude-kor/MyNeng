...
import myRef.food.MaNengDataBean;
...
public class MaNengDBBean {
	...
	public String getIngPageNum (String ingName, int pageSize) throws Exception {
		String pageNum = ""; 
		try {
			conn = ConnectionDAO.getConn();
			String sql = "select r from (select name, rownum r from ingredient) where name = '"+ ingName+"'";
			ps = conn.prepareStatement(sql); 			
			rs = ps.executeQuery();
			if (rs.next()) {
				pageNum = Integer.toString(1+(rs.getInt(1)/pageSize));
			}
		} catch(Exception ex) {
			ex.printStackTrace();
		} finally {
	...
	public String getIngPageNum_ref (String id, String ingName, int pageSize) throws Exception {
		String pageNum = ""; 
		try {
			conn = ConnectionDAO.getConn();
			String sql = "select r from (select name, rownum r from "+id+"_refrigerator) where name = '"+ ingName+"'";
			ps = conn.prepareStatement(sql); 			
			rs = ps.executeQuery();
			if (rs.next()) {
				pageNum = Integer.toString(1+(rs.getInt(1)/pageSize));
				System.out.println("출력되는 페이지 : "+pageNum);
			}
		} catch(Exception ex) {
			ex.printStackTrace();
		} finally {
	...
	public List<MaNengDataBean> getRefs(String mem_id, int startRow, int endRow) throws Exception {
		...
		try {
			...
			sql += mem_id + " order by freshness)) where r >= ? and r <= ?";			
			ps = conn.prepareStatement(sql);
			ps.setInt(1, startRow);
			ps.setInt(2, endRow);
			rs = ps.executeQuery();
	...
	public List<Integer> mixRecipe(List<MaNengDataBean> tempIngList) throws Exception { 
		...
		List<Integer> ing_idList = new ArrayList<Integer>();									// 재료ID list 생성
		for(int i = 0; i < tempIngList.size() ; i++) {
			...
			System.out.println(checkedIng_id.getIng_id());
		}
		String sql="";
		boolean check = true;
		int count = 0;
		...
		try {
			...
			if(rs.next()) {
				...
				System.out.println("레시피 총 수 : "+count);
			}											
			for(int i = 1 ; i <= count ; i++) {	
				...
				while(rs.next()) {
					pos += 1;
					if(ing_idList.contains(rs.getInt("ing_id")) ){
						check = true;
						for(int j = 0 ; j < tempIngList.size() ; j++) {
							check = true;
						MaNengDataBean list = tempIngList.get(j);						
						System.out.println("레시피 "+i+"의 "+pos+"번째 냉장고 재료"+j+1+"번째 비교");
						System.out.println("비교 재료ID : "+list.getIng_id());
						System.out.println("레시피 재료ID : "+rs.getInt("ing_id"));
						System.out.println("비교 재료 단위 : "+list.getUnit());
						System.out.println("레시피 재료 단위 : "+rs.getString("unit"));
							if(list.getIng_id()==rs.getInt("ing_id")&&list.getUnit().trim().equals(rs.getString("unit").trim())) {
								System.out.println("									재료 및 단위 일치");
								...
								check = false;
								System.out.println("									재료 및 단위 불일치");
							}
						if(check) {
							System.out.println("									"+rs.getInt("rec_id")+"의 "+rs.getInt("ing_id")+" 일치");
							ingCount += 1;
							System.out.println("									일치 하는 재료의 수 : "+ingCount);
						}
						System.out.println("									"+pos+" : 레시피 필요 재료 수 "+ingCount+" : 충족된 재료 수");
						}
						recNum = rs.getInt("rec_id");
					}else {
						check = false;
						System.out.println("									냉장고에 재료 없음");
					}
				}
				if(pos == ingCount) {
					if(!recList.contains(recNum)){
						recList.add(recNum);
						System.out.println("                                                          "+recNum+"가 입력되었습니다");
