...
public class MemberDAO {
	...
	//로그인 체크 + master3이 아닐때 true
	public boolean loginCheck(String id, String pw) {
		boolean result = false;
		try{
		conn = ConnectionDAO.getConnection();
		String sql = "select * from member where id=? and password=? and master !=3";
...
//가입시 개인냉장고 테이블 생성
	...
	//id찾기기능  이름,이메일주소, master!=3일때 
		public String findId(String name, String email) {
			String id= null;
			try {
				conn = ConnectionDAO.getConnection();
				String sql = "select id from member where name=? and email=? and master!=3";
				pstmt = conn.prepareStatement(sql);
		...
		public String findPw(String id, String name, String email){
			...
			try {
				conn = ConnectionDAO.getConnection();
				String sql ="select password from member where id=? and name=? and email=? and master!=3";
				pstmt = conn.prepareStatement(sql);
				pstmt.setString(1, id);
		...
		//탈퇴회원 체크 탈퇴회원 ->true
		public boolean masterCheck(String id) {
			boolean result = false;
			try{
			conn = ConnectionDAO.getConnection();
			String sql = "select * from member where id=? and master=3";
			pstmt= conn.prepareStatement(sql);
			pstmt.setString(1, id);
			rs=pstmt.executeQuery();
			if(rs.next()) {
				result = true; //상태가 3임 -> 탈퇴회원
			}
			}catch(Exception e) {e.printStackTrace();}
			finally {ConnectionDAO.close(rs, pstmt, conn);}
			return result;
	...
	public MemberDTO getMember2(String id) {
		...
		try {
			...
			if(rs.next()){
				...
				dto.setAddress(rs.getString("address"));
				dto.setGroup_pw(rs.getString("group_pw"));
				dto.setGroup_id2(rs.getString("group_id"));
			}
