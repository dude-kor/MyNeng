package shoppingInfoShare.comment;

import java.sql.*;
import java.util.*;
import dbConnectionDAO.ConnectionDAO;
import shoppingInfoShare.comment.CommentDataBean;
...
public class CommentDBBean 
{
	...
	public int insertArticle_comment(CommentDataBean article, int comment_listNum, int comment_pageNum) throws Exception {
		if (comment_listNum == 0)
		{
			comment_listNum = 0;
		}
		else if(comment_listNum != 0)
		{
			comment_listNum = comment_listNum + ((comment_pageNum-1)*10);
		}
		...
		try {
			if(comment_listNum == 0)
			{
			String tableNum = "sis_comment_"+article.getNum();
			...
			// 댓글에 대한 댓글 
						else
						{
							...
							sql = sql = "select comment_id,comment_text,reg_date,ref,re_step,re_level,r "
									+"from (select comment_id,comment_text,reg_date,ref,re_step,re_level,rownum r "
								     +"from (select comment_id,comment_text,reg_date,ref,re_step,re_level "
								            +"from "+tableNum+" order by ref asc, re_step asc) order by ref asc, re_step asc ) "
	...
	public int deleteArticle_comment(int num,int listNum, String id,int comment_pageNum) throws Exception
	{
		...
		int x = -1;
		String seqName = "sis_comment_"+num+"_seq";
		String tableName = "sis_comment_"+num;
		try 
		{
			conn = ConnectionDAO.getConnection();
	...
	public int getArticleCount_comment(int tableNum) throws Exception
	{
		...
		int x = 0;
		String sql ="";
		String tableName = "sis_comment_"+tableNum;
		String seqName = tableName+"_seq";
		String searchTable = "SIS_COMMENT_"+tableNum;
		try
		{
			conn = ConnectionDAO.getConnection();
