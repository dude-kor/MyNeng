...
<%	}
	...
	HashMap<Integer, Integer> scrapcount = manager.getScrapCount();
%>
    ...
    <tr height="30" bgcolor="#d6cabc">
    	...
    	<td align="center" width="50">찜횟수</td>
	...
	if(count != 0){
		for(int i = 0; i < recipeList.size(); i++){
	    	...
	    	<td align="center" width="300">
	    		<a href="../recipe/recipeContentForm.jsp?num=<%=recipe.getNum()%>&pageNum=<%=currentPage%>&random_id=0&comment_listNum=0">
	    	...
	    	<td align="center" width="50"><%=scrapcount.get(recipe.getNum()) %></td>
	    </tr>		
	<%		
		number--;}
		}
...
<%	} %>
	...
	<% 
		if(count > 0){
			...
			if(startPage > 10){%>
				<a href="recipeScrapTotal.jsp?pageNum=<%=startPage - 10 %>">[이전]</a>	
<% 			}
			for(int i = startPage; i <= endPage; i++){ %>
				<a href="recipeScrapTotal.jsp?pageNum=<%=i %>">[<%= i%>]</a>	
<% 			}
			if(endPage < pageCount){%>
				<a href="recipeScrapTotal.jsp?pageNum=<%=startPage + 10 %>">[다음]</a>
<% 			}
		}
	/* System.out.println(scrapcount);
	System.out.println(manager.getallRecipeNum());
	System.out.println(manager.getallMemberId());
	for(int i = 0; i < manager.getallRecipeNum().size(); i++) {
		System.out.println(manager.getallRecipeNum().get(i));
	} */
