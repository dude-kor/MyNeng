...
<%@ page import = "groupBuying.comment.CommentDBBean" %>
<%@ page import = "groupBuying.comment.CommentDataBean" %>
...
<%
    if (count > 0) {
        ...
        if (startPage > 10) {    %>
        <a href="content.jsp?num=<%=num %>&pageNum=<%=pageNum %>&comment_pageNum=<%= startPage - 10 %>&comment_listNum=0">[이전]</a>
<%      }
        for (int i = startPage ; i <= endPage ; i++) {  %>
        	<a href="content.jsp?num=<%=num %>&pageNum=<%= pageNum %>&comment_pageNum=<%=i %>&comment_listNum=0">[<%= i %>]</a>
<%		}
        if (endPage < pageCount) {  %>
        	<a href="content.jsp?num=<%=num %>&pageNum=<%=pageNum %>&comment_pageNum=<%= startPage + 10 %>&comment_listNum=0">[다음]</a>
<%		}
    }
%>
</center>
