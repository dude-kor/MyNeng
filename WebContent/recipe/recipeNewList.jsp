...
<%
	...
	if(master == 2){
		count = dao.getRecipeCount(today); 	// 작성된 게시글 숫자
		if(count > 0){
			recipeList = dao.getRecipes(startRow, endRow, today);
		}
	}
	number = count-(currentPage-1)*pageSize;
%>
	<center> <b>오늘 작성된 레시피</b><br />
			<b>글목록(전체 글: <%=count %>)</b>
...
<%
	if(count != 0){
		for(int i = 0; i < recipeList.size(); i++){
	    	...
	    	<td align="center" width="300">
	    		<a href="/myneng/recipe/recipeContentForm.jsp?num=<%=recipe.getNum()%>&pageNum=<%=currentPage%>&random_id=0&comment_listNum=0">
	...
	<%		}
	    ...
	    <tr><td colspan="10" align="center">오늘 작성된 레시피가 없습니다.</td></tr>
