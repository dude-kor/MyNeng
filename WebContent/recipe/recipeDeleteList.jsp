...
<%
	...
	if(master == 2){							// 관리자일때
		count = dao.getRecipeCount(0); 	// 삭제된 게시글 숫자
		if(count > 0){
			recipeList = dao.getRecipes(startRow, endRow, 0);
		}
	}
	number = count-(currentPage-1)*pageSize;
%>
	<center> <b>글목록(삭제된 글: <%=count %>)</b>
...
<%	} %>
	...
	<% 
		if(count > 0){
			...
			if(startPage > 10){%>
				<a href="recipeDeleteList.jsp?pageNum=<%=startPage - 10 %>">[이전]</a>	
<% 			}
			for(int i = startPage; i <= endPage; i++){ %>
				<a href="recipeDeleteList.jsp?pageNum=<%=i %>">[<%= i%>]</a>	
<% 			}
			if(endPage < pageCount){%>
				<a href="recipeDeleteList.jsp?pageNum=<%=startPage + 10 %>">[다음]</a>
<% 			}
		}
