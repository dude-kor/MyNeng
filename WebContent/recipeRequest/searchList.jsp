...
<%@ page import = "recipeRequest.board.BoardDataBean" %>   
			...
			<tr>
				<td align="right" bgcolor="<%=value_c%>">
					...
					{%>
						...
						<a href="recipeRequest.jsp">전체 목록</a>
					<%}
					else
					{%>
...
<%
    if (count > 0) {
        ...
        if (startPage > 10) {    %>
        <a href="recipeRequest.jsp?pageNum=<%= startPage - 10 %>">[이전]</a>
<%      }
        for (int i = startPage ; i <= endPage ; i++) {  %>
        	<a href="searchList.jsp?col=<%=col %>&search=<%=search %>&pageNum=<%= i %>">[<%= i %>]</a>
<%		}
        if (endPage < pageCount) {  %>
        	<a href="recipeRequest.jsp?pageNum=<%= startPage + 10 %>">[다음]</a>
<%		}
    }
%>
